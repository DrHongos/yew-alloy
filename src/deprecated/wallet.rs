...
//use crate::local_eip1193::Provider as NewProvider;
use ethers_web::Ethereum;

/* 
TODO:

- check out this https://github.com/tomusdrw/rust-web3/blob/2711cd00d51bbfa6be1996cebdd991f7ed77115c/src/transports/eip_1193.rs
...
- error handling
- check EIP6963
- parse alloy-provider -> custom Transport

    - implement custom transport? -> provider
    - try to get it from the JsValue of the window.ethereum provider



https://github.com/nlordell/walletconnect-rs/blob/master/src/transport.rs
...
impl Component for Wallet {
    ...
    fn create(ctx: &Context<Self>) -> Self {
        ...
        let p = Ethereum::default().expect("Error creating provider");
        log(format!("provider {:#?}", &p).as_str());
        Self {
            account: None,
            chain: None,
            version: None,
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            WalletMsg::ConnectMetamask => {
                ...
                //log(&format!("Provider {:#?}", &provider));
                let link = ctx.link().clone();
                
                provider.request_call(
